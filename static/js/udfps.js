...
async function obtainCalibration(setProgress) {
    await ensureConnected(setProgress);
    let product = await device.getVariable("product");
    if (!supportedDevices.includes(product)) {
        throw new Error(`device model (${product}) is not supported`);
    }
    let response = await device.runCommand("oem udfps get-cal-url");
    let url = response.text.replace(/(\r\n|\n|\r)/gm, "");
    calibrationFile = url.split("/").pop();
    console.log(`Calibration file url: ${url}`);
    await blobStore.init();
    await blobStore.download(url, (progress) => {
        setProgress(`Downloading ${calibrationFile}...`, progress);
    });
    setProgress(`Downloaded ${calibrationFile} calibration file.`, 1.0);
}

async function flashCalibration(setProgress) {
    await ensureConnected(setProgress);
    // fastboot stage <blob>
    await blobStore.init();
    let blob = await blobStore.loadFile(calibrationFile);
    if (blob === null) {
        throw new Error("You need to obtain calibration file first!");
    }
    console.log(`Loaded ${calibrationFile} from blobStore`);
    setProgress(`Uploading ${calibrationFile} to the device...`);
    await device.stagePayload(calibrationFile, blob);
    setProgress("Updating calibration...");
    await device.runCommand("oem udfps update-cal");
    setProgress("Rebooting to the bootloader...");
    await device.reboot("bootloader", 4000, reconnectCallback);
    return `Updated calibration (${calibrationFile}) successfully.`;
}
